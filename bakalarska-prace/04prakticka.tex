...
Cílem praktické části je prozkoumat dva typy modelů. První typ modelu bude predikovat výhru zápasu pro jednotlivé hráče pro jednotlivé mapy. Hlavním cílem 
...
Každý model je trénován na 80 \% dat. Pro interpretaci významnosti prediktorů je použit Waldův test, který je v posledním sloupci $Pr(>|z|)$ každé tabulky.
Alfa, které se také říká hladina významnosti, je stanovena jako $\alpha = 0,05$. Pro matici záměn je model otestován na validačních datech, 
...
Pro hráče budou sestaveny logistické modely z celého datového souboru přes kategorie map. Modely budou následně porovnány pomocí statistik vycházejících
z matice záměn
...
neznali. Modely budou porovnánu podle přesnosti predikce a podle toho, jaké prediktory jsou pro model významné.

Modely pro týmy budou sestavené nejprve z celého datového souboru a následně pro dva vybrané referenční týmy. První tým, pro který se vytvoří model, bude tým Astralis.
...
agregaci bude použit aritmetický nebo geometrický průměr. Aritmetický průměr bude použit u prediktorů, u kterých dává smysl jejich úhrn. Může to být například počet zabití 
nebo počet smrtí. Geometrický průměr je pak použit u prediktorů, kde dává smysl součin, což může být například procento zabití nepřátel do hlavy.
...
Datový soubor \textit{players.csv} obsahuje charakteristiky
...
\newpage
Transformovaný datový soubor má 10 proměnných. Interpretace je následující:
\begin{itemize}
...
Histogramy prediktorů z obrázku \ref{fig:histogram_prediktoru} ukazuje, že přediktory \textit{rating}, \textit{hs}, \textit{kills} a \textit{deaths} mají normální rozdělení
