...
picks <- read_csv("data/picks.csv", show_col_types = FALSE,
                  col_select = -c(date, match_id, event_id, inverted_teams, system)) %>%
  filter(!is.na(best_of)) %>%
  mutate(
    across(t1_removed_1:left_over, ~ ifelse(.x == '0.0', NA, .x)),
    across(.cols = c(-best_of), .fns = create_factor),
...
```{r resluts-fx}
# results.raw <- 
results <- read_csv('data/results.csv', show_col_types = F,
                    col_select = c(team_1, team_2, map = '_map', map_winner, rank_1, rank_2,
                                   starting_ct)) %>% 
...
data <- results %>%
  select(-map) %>%
  select(-team_2, -rank_2) %>%
  rename(team = team_1, rank = rank_1) %>%
  rbind(results %>% select(-team_1, -rank_1) %>% rename(team = team_2, rank = rank_2)) %>%
  mutate(map_winner = as.numeric(team == map_winner),
         starting_ct = as.numeric(starting_ct == team)) %>%
  select(-team)

model <- glm(map_winner ~ ., data = data, family = binomial)
summary(model)
```
