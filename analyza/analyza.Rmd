...
```{r }
g <- data |>
  mutate(cat = ifelse(!team %in% c("Astralis", "eSuba"),
                      "Všechny záznamy",
                      paste0("tým ", team)),
         cat = factor(cat,
...
for (t in teams) {
  ...
  model.data <- data |> 
    ...
    summarise(
      ...
      map_winner = unique(map_winner) |> as.factor(),
      starting_ct = unique(starting_ct) |> as.factor(),
      run_mean_3_months = unique(run_mean_3_months),
      .groups = "drop" 
      )
  ...
  model <- get_team_model(
    ...
    .x = paste0("mean_kills + mean_assists + mean_deaths + mean_hs +",
                "mean_fkdiff + run_mean_3_months + map:starting_ct")
  )
