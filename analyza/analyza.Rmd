...
get_player_model <- function(.data, .map, .formula = NA) {
  if (is.na(.formula)) {
    .formula = map_winner ~ kills + assists + deaths + hs + fkdiff +
      starting_ct
  }
...
get_matrix_for_player_model <- function(.model, .validate) {
  confusionMatrix(
...
```{ <-  players.models, options}
players.models <- list()
players.matrixes <- list()

for (map in maps) {
  set.seed(48754)
  split <- createDataPartition(data$map_winner,
                               p = 0.8,
                               list = F)
  train <- data[split, ]
  validate <- data[-split, ]
  model <-  get_player_model(.data = train,
                             .map = map)
  
  players.models[[map]] <- model
  players.matrixes[[map]] <-
