...
g <- players |> 
  count(map) |> 
  # Every map has 10 records (10 players), dividing normalizes
  # the count
  mutate(n = n/10) |> 
  ...
  labs(
    ...
    x = "Mapa",
    y = "Počet zápasů",
    caption = "Datový soubor: players.csv"
  )
...
g <- players |>
  ...
  labs(
    title = "Distribuce zabití v zápasech",
    subtitle = sprintf("Průměr: %.2f, Medián: %.2f", mean, median),
    x = "Počet zabití",
    y = "Absolutní čestnost",
    caption = "Datový soubor: players.csv",
    linetype = "Statistika",
    colour = "Statistika"
  )
...
g <- players |> 
  ...
  labs(
    ...
    x = "Mapa",
    y = "Rating",
    caption = "Datový soubor: players.csv, results.csv"
  )
...
g <- players |> 
  ...
  labs(
    ...
    subtitle = "Prediktory: kills, assists, deaths, hs, fkdiff, rating",
    caption = "Datový soubor: players.csv"
  )
...
model.per_player.dust2 <- glm(
  map_winner ~ kills + assists + deaths + hs + fkdiff + 
    team_rank + run_mean_3_months,
  data = model.data.per_player.dust2,
  family = binomial
)

save_bc_model(model.per_player.dust2,
              "model.per_player.dust2.tex")
...
save_bc_model(model.per_player.inferno,
              "model.per_player.inferno.tex")
...
### Matice záměn
Pro první vyhodnocení se použíje matice záměn
```{r model.per_player.conf}
model.per_player.dust2.conf <- confusionMatrix(
  data = predict(model.per_player.dust2, type = "response") |> 
    round() |> 
    as.factor(),
  reference = model.data.per_player.dust2$map_winner |> 
    as.factor(),
  positive = "1"
) |>
  tidy(by_class = TRUE) |> 
...
model.per_player.inferno.conf <- confusionMatrix(
  data = predict(model.per_player.inferno, type = "response") |> 
    round() |> 
    as.factor(),
  reference = model.data.per_player.inferno$map_winner |> 
    as.factor(),
  positive = "1"
) |>
  tidy(by_class = TRUE) |> 
...
g <- bind_rows(
  model.per_player.dust2.conf,
  model.per_player.inferno.conf
) |> 
  ggplot(aes(x = map, y = estimate, color = term)) +
  geom_point() +
  geom_text(aes(x = map, y = estimate, label = round(estimate, 3)),
            nudge_x = 0.1) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) +
  scale_y_continuous(breaks = seq(0.770, 0.810, by = 0.005),
                     limits = c(0.77, 0.810)) +
  theme(
    panel.grid.major.x = element_blank(),
    panel.grid.minor.y = element_blank(),
  ...
  scale_color_discrete(
    labels = c("accuracy" = "Přesnost", "sensitivity" = "Citlivost")
  ) +
  labs(
    title = "Odhadované statistiky z matice záměn",
    subtitle = "Osa Y je přiblížena pro lepší vyobrazení",
    x = "Modelovaná mapa",
    y = "Hodnota statistiky",
    color = "Statistika",
    caption = "Modely pro předpověd výhry na základě statistik hráčů"
  )

save_bc_picture(g, f = "model.per_player.conf.png")
g
```

### Waldův test
```{r model.per_player.wald}
model.per_player.dust2.wald <- model.per_player.dust2 |> 
  tidy() |> 
  filter(term != "(Intercept)") |> 
  select(term, p.value) |> 
  mutate(map = "Dust2")

model.per_player.inferno.wald <- model.per_player.inferno |> 
  tidy() |> 
  filter(term != "(Intercept)") |> 
  select(term, p.value) |> 
  mutate(map = "Inferno")
...
g <- bind_rows(
  model.per_player.dust2.wald,
  model.per_player.inferno.wald
) |>
  mutate(p.value = round(p.value, 5)) |> 
  group_by(term) |> 
  filter(any(p.value >= 0.05)) |> 
  ungroup() |> 
  ggplot(aes(x = map, y = p.value, color = term)) +
  geom_point() +
  geom_text(aes(x = map, y = p.value, label = round(p.value, 4)),
            nudge_x = 0.2) +
  theme(
    panel.grid.major.x = element_blank(),
    panel.grid.minor.y = element_blank(),
  ...
  labs(
    title = "Prediktory, které se pro modely liší",
    subtitle = "Osa Y je přiblížena pro lepší vyobrazení. Hodnoty p jsou zaokrouhlené.",
    x = "Modelovaná mapa",
    y = "Hodnota p",
    color = "Prediktor",
    caption = "Modely pro předpověd výhry na základě statistik hráčů"
...
save_bc_picture(g, f = "model.per_player.wald.png")
g
```
