...
save_bc_picture(g, "histogram_prediktoru.png")
g
```
...
save_bc_picture(g, "sloupce_podle_strany.png")
g
```
...
model_for_player_map <- function(.tbl, .map,
                                 formula = map_winner ~ .) {
  set.seed(1234)
  .tbl |>
...
for (i in 1:length(maps)) {
  map <- maps[i]
  model <- model_for_player_map(data, map)
  model.players[[map]] <- model
...
save_bc_model(model.players$Mirage, "mirage_pro_hrace.tex")
```

### Vertigo
```{r vertigo_pro_hrace.tex}
model <- model.players$Vertigo
save_bc_model(model, "vertigo_pro_hrace.tex")

model.players$Vertigo_opt <- model_for_player_map(data, "Vertigo",
                              map_winner ~ kills + assists + deaths)
save_bc_model(model.players$Vertigo_opt, "vertigo_pro_hrace_opt.tex")
```

# Matice záměň
```{r data.players.conf}
rownames = c("Výhra", "Prohra")
colnames = c("Predikovaná Výhra", "Predikovaná prohra")

conf_for_player_map <- function(.model) {
  data.players.conf[["Mirage"]] <- confusionMatrix(
    data = predict(.model, type = "response") |>
      round() |>
      as.factor(),
    reference = .model$data$map_winner |>
      as.factor(),
    positive = "1"
  )
}

data.players.conf <- list()
data.players.conf[["Mirage"]] <-
  conf_for_player_map(model.players$Mirage)

data.players.conf[["Vertigo"]] <-
  conf_for_player_map(model.players$Vertigo_opt)


dimnames(data.players.conf$Mirage$table) <- 
  list(
    rownames,
    colnames
  )

data.players.conf$Mirage$table |>
  kbl(
    format = "latex",
    align = "c",
    valign = "t",
