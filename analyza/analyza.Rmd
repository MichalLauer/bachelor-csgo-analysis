...
for (i in 1:length(maps)) {
  map <- maps[i]
  model.players[[map]] <- model_for_player_map(data, map)
  
  if (!map %in% c("Vertigo", "Mirage")) next
  sprintf("============ Map: %s", map) |> 
    print()
  print(model.players[[i]] |> summary())
}
```

## Model pro tým
```{r }
model_for_team_map <- function(.tbl, .map) {
  set.seed(1234)
  .tbl |>
    filter(map == .map) |> 
    select(-map) |> 
...
for (i in 1:length(maps)) {
  map <- maps[i]
  model.players[[map]] <- model_for_player_map(data.model, map)
